...
class TaskSerializer(serializers.ModelSerializer):
    ...
    responsible_id = serializers.PrimaryKeyRelatedField(
        queryset=User.objects.all(), source='responsible',
        write_only=True, required=False, allow_null=True
    )
    images = TaskImageSerializer(many=True, read_only=True)

    # цвет бейджа для выполнённых задач
    done_color = serializers.SerializerMethodField()
    ...
    def get_done_color(self, obj):
        if obj.column != 'done':
            return ''
        if not obj.completed_at:
            return 'bg-gray border border-gray text-gray-700'
        ...
        if obj.completed_at <= obj.due_date:
    ...
    def validate(self, attrs):
        ...
        if responsible and project and not project.participants.filter(id=responsible.id).exists():
